...
void LmacUpdateTx(/* input */ BOOL_T TX_WE, /* input */ PHY_DATA_T TX_DATA, PHY_DATA_T* out) {
  // helpers
  ...
  // TX_PKT_RD_CNT
  PHY_DATA_T size_load_from_buffer = out[TX_BUFF_RD_PTR];
  ...
  /* Update states */
  out[TX_BUFF_WR_PTR] = in_data_val;
...
void LmacMmioUpdateInstr1(/* input */ ADDR_T MMIO_ADDR_IN,
  ...
  if (LmacDecodeTx(0x1 /* TX_WE */, chunk /* TX_DATA */)) {
    LmacUpdateTx(0x1 /* TX_WE */, chunk /* TX_DATA */, out);
  }

  return;
}

void LmacMmioWriteL(/* input */ PKT_DATA_T data_in, PHY_DATA_T* out) {
  ADDR_T addr_in = 0xff00;

  if (LmacMmioDecodeInstr0(addr_in, data_in, true)) {
    LmacMmioUpdateInstr0(addr_in, data_in, true, out);
  } else if (LmacMmioDecodeInstr1(addr_in, data_in, true)) {
    LmacMmioUpdateInstr1(addr_in, data_in, true, out);
  }
